...
We modelled three types of catch events with different average catch rates:
...
3.  A rare bycatch species that is caught inconsistently, but is a vulnerable species and/or a designated endangered, threatened, or protected species (e.g. green turtles). The rare bycatch species could also be representative of other rare events, such as trans-shipment
...
We explored three protocols for allocating monitoring and review of trips. (1) Trips selected at random. (2) Bias towards trips with lower than average catch rates. This simulates situations where captains attempt to decrease the probability of catching bycatch when fishing activity is under human observation. (3) Bias towards trips with lower than average catch rates and high variability in catch rates across trips. This is similar to (2), except the captains have greater flexibility to modify their bycatch when observers are on board. For example, they may fish in different regions, use different gear, or go on shorter trips.
...
We explored three protocols for allocation of monitoring and review to vessels (that mirrored the three options under Scenario 2). (1) Vessels selected at random. (2) Bias towards vessels with lower than average catch rates. This simulates situations where a nation or fishery attempt to influence the data by deploying cameras on vessels with the best practices . (3) Bias towards vessels with lower than average catch rates and high variability in catch rates across vessels. This is similar to (2), except the fishery has greater variability in catch rates across vessels. For example, different vessel captains may have different levels of knowledge about avoiding bycatch or use different fishing gears.

### Simulations

Our aim was to explore the likelihood that the fictitional monitoring scenarios would obtain unbiased catch rates when applied to each of the three catch event categories. We simulated a 1000 replicate years of fishing, to capture the full range of variation described for the catch rates. For each of the 1000 replicates we applied each of the three monitoring scenarios along with their respective allocation protocols.
...
*This is a good start. It needs a few sentences with citations on how biased logbook data used in stock assessments, quota allocation, etc. can have detrimental impacts on fishery sustainability. And then this will flow into a few sentences on how improved independent monitoring can be used to improve logbook accuracy and therefore all the downstream products that use logbook data*
...
All parameters were chosen to be representative of the Western Pacific longline tuna fleets, data given in @brown2021electronic, parameters are in Table S3. The analysis was performed with the R program [@Rprogram]. The code are available online at <https://github.com/cbrown5/msc-review-rates>.
...
**Table S2** Model Equations and parameters of the monitoring module **Table S2** Model Equations and parameters of the monitoring module
...
| p_monitor | 0.3 | Proportion of sets monitored (e.g. 20%, 30%), as specified by MSC requirements |

**Table S4** Parameters for monitoring scenarios. Parameters was based on values from @brown2021electronic.

| Scenario | Parameter | Value | Description |
|------------------|------------------|------------------|--------------------|
| Baseline - YFT | β₀ | 5.6 | Baseline catch rate for common market species (yellowfin tuna) |
|  | σˣ | 0.41 | Standard deviation of vessel-level variation in catch rates |
|  | σᶻ | 0.67 | Standard deviation of trip-level variation in catch rates |
|  | $\text{bias}_{vessels}$ | 0 | No bias in vessel selection (random) |
|  | $\text{bias}_{trip}$ | 0 | No bias in trip selection (random) |
| High vessel bias | β₀ | 5.6 | Same baseline catch rate as scenario 1 |
|  | σˣ | 0.41 | Standard vessel-level variation |
|  | σᶻ | 0.67 | Standard trip-level variation |
|  | $\text{bias}_{vessels}$ | -2 | Bias towards vessels with lower catch rates |
|  | $\text{bias}_{trip}$ | 0 | No bias in trip selection |
| High trip bias | β₀ | 5.6 | Same baseline catch rate as scenario 1 |
|  | σˣ | 0.41 | Standard vessel-level variation |
|  | σᶻ | 0.67 | Standard trip-level variation |
|  | $\text{bias}_{vessels}$ | 0 | No bias in vessel selection |
|  | $\text{bias}_{trip}$ | -2 | Strong bias towards trips with lower catch rates |
| High vessel variance + vessel bias | β₀ | 5.6 | Same baseline catch rate |
|  | σˣ | 0.82 | Double the vessel-level variation |
|  | σᶻ | 0.67 | Standard trip-level variation |
|  | $\text{bias}_{vessels}$ | -2 | Strong bias towards vessels with lower catch rates |
|  | $\text{bias}_{trip}$ | 0 | No bias in trip selection |
| High trip variance + trip bias | β₀ | 5.6 | Same baseline catch rate |
|  | σˣ | 0.41 | Standard vessel-level variation |
|  | σᶻ | 1.3 | Nearly double the trip-level variation |
|  | $\text{bias}_{vessels}$ | 0 | No bias in vessel selection |
|  | $\text{bias}_{trip}$ | -2 | Strong bias towards trips with lower catch rates |
| Rare species + vessel bias | β₀ | 0.08 | Much lower baseline catch rate for rare bycatch species |
|  | σˣ | 0.55 | Higher vessel-level variation for rare species |
|  | σᶻ | 0.65 | Similar trip-level variation |
|  | $\text{bias}_{vessels}$ | -2 | Strong bias towards vessels with lower catch rates |
|  | $\text{bias}_{trip}$ | 0 | No bias in trip selection |
| Very rare species + vessel bias | β₀ | 0.008 | Very low baseline catch rate for very rare species |
|  | σˣ | 0.55 | Higher vessel-level variation |
|  | σᶻ | 0.65 | Similar trip-level variation |
|  | $\text{bias}_{vessels}$ | -2 | Strong bias towards vessels with lower catch rates |
|  | $\text{bias}_{trip}$ | 0 | No bias in trip selection |
| Rare species + trip bias | β₀ | 0.08 | Low baseline catch rate for rare species |
|  | σˣ | 0.55 | Higher vessel-level variation |
|  | σᶻ | 0.65 | Similar trip-level variation |
|  | $\text{bias}_{vessels}$ | 0 | No bias in vessel selection |
|  | $\text{bias}_{trip}$ | -2 | Strong bias towards trips with lower catch rates |
| Very rare species + trip bias | β₀ | 0.008 | Very low baseline catch rate |
|  | σˣ | 0.55 | Higher vessel-level variation |
|  | σᶻ | 0.65 | Similar trip-level variation |
|  | $\text{bias}_{vessels}$ | 0 | No bias in vessel selection |
|  | $\text{bias}_{trip}$ | -2 | Strong bias towards trips with lower catch rates |
