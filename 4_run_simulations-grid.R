...
# Run a grid of review rates, with 100% coverage 
...
#Modify monitoring scenarios. Keep those with p_monitor ==1, then replicate for p_sets_select seq(0.1, 1, by=0.1)
monitoring_params_df <- monitoring_params_df %>%
  filter(p_monitor == 1) %>%
  select(-p_sets_select, -p_monitor) %>%
  crossing(p_sets_select = seq(0.1, 1, by = 0.1), p_monitor = c(0.3, 1)) %>%
  mutate(description = paste0(description, "_", "psets", p_sets_select))
...
write.csv(all_results_df, "outputs/simulation_results_full_tidy-grid.csv", row.names = FALSE)
write.csv(summary_results_df, "outputs/simulation_results_summary_tidy-grid.csv", row.names = FALSE)
